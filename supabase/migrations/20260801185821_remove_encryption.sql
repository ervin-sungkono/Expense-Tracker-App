drop function if exists public.create_space(text, text);
drop function if exists public.apply_space_mutations(jsonb);
drop function if exists public.accept_space_invitation(text, uuid, text, text);

drop table if exists public.space_member_keys;
drop table if exists public.user_public_keys;
drop table if exists public.space_assets;

alter table public.spaces
    drop column if exists current_key_version cascade,
    drop column if exists pending_key_version cascade,
    drop column if exists rotation_status cascade,
    drop column if exists rotation_started_at cascade;

alter table public.space_members drop constraint if exists space_members_status_check;
update public.space_members set status = 'revoked' where status = 'revoked_pending_rotation';
alter table public.space_members
    add constraint space_members_status_check check (status in ('active', 'revoked'));

delete from public.space_records;

alter table public.space_records
    drop column if exists ciphertext cascade,
    drop column if exists iv cascade,
    drop column if exists crypto_version cascade,
    drop column if exists payload_schema_version cascade,
    drop column if exists space_key_version cascade,
    add column payload jsonb;

alter table public.space_records
    add constraint space_records_payload_required
    check (deleted_at is not null or payload is not null);

alter table public.space_invitations
    drop column if exists encrypted_space_key,
    drop column if exists key_iv,
    drop column if exists space_key_version;
...
begin
    ...
    insert into public.spaces (owner_id, name)
    values (caller_id, trim(space_name))
    returning * into created_space;

    insert into public.space_members (space_id, user_id, role)
    values (created_space.id, caller_id, 'admin');
...
begin
    ...
    for item in select * from jsonb_array_elements(mutations) loop
        ...
        if item ->> 'operation' = 'delete' then
            ...
            set payload = null, deleted_at = now(), last_mutation_id = (item ->> 'mutation_id')::uuid
            where id = (item ->> 'record_id')::uuid
            returning * into existing;
        elsif existing.id is null then
            insert into public.space_records (
                id, space_id, entity_type, payload, last_mutation_id, created_by, updated_by
            ) values (
                (item ->> 'record_id')::uuid, (item ->> 'space_id')::uuid,
                item ->> 'entity_type', item -> 'payload', (item ->> 'mutation_id')::uuid,
...
begin
    ...
    insert into public.space_members (space_id, user_id, role)
    values (invitation.space_id, recipient_user_id, invitation.role)
    returning * into membership;

    update public.space_invitations
    set accepted_by = recipient_user_id, accepted_at = now()
...
revoke execute on function public.accept_space_invitation(text, uuid, text)
