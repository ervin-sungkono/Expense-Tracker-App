  ...
  if (!categoryData) {
    return (
      <div className="mb-4">
        <SubHeader
          loading
          title="Categories"
          description={"based on this month's remaining budget"}
          link="/categories"
        />
    ...
    return (
      <div className="mb-4">
        <SubHeader
          title="Categories"
          description={"based on this month's transaction"}
        ...
        {categoryData.length > 0 ? (
            ...
            items={categoryData
              ...
              .map(category => ({
                ...
                component: (
                  <CategoryCard
                    {...category}
                    slug={`/transactions?category=${encodeURIComponent(category.name)}`}
                  />
                ),
              }))}
          />
          ...
          <div className="h-40 flex flex-col justify-center items-center gap-4 w-full bg-neutral-200 dark:bg-neutral-800 rounded-lg py-6 px-4">
            <p className="text-sm md:text-base text-center font-medium text-dark/80 dark:text-white/80">
              No category found, please create a new category
            </p>
            {canManageSpace && (
