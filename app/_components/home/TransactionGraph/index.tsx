...
export default function TransactionGraph({
  ...
  const transactionDiff =
    historyTotalTransaction != null
      ? ((totalTransaction - historyTotalTransaction) / historyTotalTransaction) * 100
      : 0;
  ...
  useEffect(() => {
    if (
      historyLabels &&
      historyTransactionData &&
      historyTransactionData[transactionType] &&
      historyTransactionData[transactionType].length > 0
    ) {
          ...
          x: {
            ...
            ticks: {
              ...
              callback: function (value) {
                ...
                if (type === 'MONTHLY' || type === 'WEEKLY')
                  return new Date(labelValue).toLocaleDateString('en-US', {
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric',
                  });
    ...
    return (
      <div className="flex justify-center items-center h-48 xs:h-64 px-3 py-4">
        <p className="text-sm md:text-base text-center text-dark dark:text-white">
          No transaction data found, please create an transaction first.
        </p>
      </div>
    );
  return (
    <div className="flex flex-col justify-center items-start min-h-48 xs:min-h-64">
      {chartData && chartOptions ? (
        <p className="w-full text-center mt-1.5 font-semibold text-sm md:text-base">
          ...
          <span className={`ml-2 ${transactionDiff >= 0 ? 'text-ocean-blue' : 'text-red-400'}`}>
            {transactionDiff < 0 ? '-' : '+'}
            {Math.abs(transactionDiff).toFixed(2)}%
          </span>
        </p>
      ) : (
