...
function CategoryCard({ name, type, total = 0, slug = '' }) {
  ...
  return (
    <Link
      scroll={false}
      href={slug}
      className={`h-28 flex flex-col items-end px-4 py-3 text-white ${getCategoryType() ? 'bg-success-gradient hover:bg-success-gradient--hover' : 'bg-danger-gradient hover:bg-danger-gradient--hover'} transition-colors duration-300 ease-in-out rounded-lg`}
    >
