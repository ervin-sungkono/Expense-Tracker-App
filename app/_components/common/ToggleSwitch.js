...
export default function ToggleSwitch({ ref, icon, switchStatus = false, onStatusChange }) {
    ...
    return(
        <div ref={ref} onClick={toggle} className={`relative wrapper w-12 h-7 md:w-14 md:h-8 p-1 ${status ? 'bg-ocean-blue' : 'bg-neutral-300 dark:bg-neutral-600'} rounded-full transition-colors duration-200 ease-in-out`}>
