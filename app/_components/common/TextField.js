    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="relative flex flex-col gap-1">
        ...
        <div className="flex items-center gap-1">
          {errorMessage && (
            <p className="text-[10.8px] md:text-xs text-red-600 dark:text-red-400 grow">
              {errorMessage}
            </p>
          )}
          {maxLength && (
            <p
              className={`text-xs  ml-auto ${value.length > maxLength ? 'text-red-600/80 dark:text-red-400/80' : 'text-dark/80 dark:text-white/80'}`}
            >
