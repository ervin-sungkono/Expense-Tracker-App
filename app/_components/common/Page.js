...
export default function Page({ children, title, show = false, hideFn = null, hideBackButton = false }) {
    ...
    useEffect(() => {
        let hide = null;
        if(!show && !hidden) hide = setTimeout(() => setHidden(true), 400);
        else if(show && hidden) {
            setHidden(false)
        }
    ...
    return(
        <div className="fixed w-full h-full top-0 left-0 right-0 bottom-0 flex justify-center items-center z-[10000]">
            <div className={`relative w-full h-full flex flex-col overflow-y-auto max-w-2xl bg-background px-6 pb-8 z-50 ${show ? 'animate-[slide-in_.4s_forwards_ease-in-out]' : 'animate-[slide-out_.4s_forwards_ease-in-out]'}`}>
