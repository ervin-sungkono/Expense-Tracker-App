            ...
            <div className="w-full flex justify-start gap-0.5">
                {contents.map(content => (
                    <div 
                        ...
                        className={`cursor-pointer py-1.5 px-2.5 rounded-t-lg font-semibold text-dark dark:text-white border ${selectedTab === content.id ? 'bg-basic-gradient border-transparent' : 'border-ocean-blue'}`}
                    >
                        {content.label}
                    </div>
                ))}
