...
export default function SelectField({
  ...
  useEffect(() => {
    if (dropdownRef?.current && showOption && selected?.id) {
      ...
      dropdownRef.current
        .querySelector(`#${name}-${selected.id}`)
  ...
  return (
    <div className="flex flex-col gap-2">
      {label && (
        <label
          htmlFor={`id-${name}`}
          className="block font-semibold text-xs md:text-sm text-dark-blue"
        >
          {label}
      ...
      <div className="relative">
        <input name={name} type="text" defaultValue={selected?.id} readOnly hidden />
        <div
          id={`id-${name}`}
        ...
        {!overrideOnClick && (
          <div className={`${showOption ? 'block' : 'hidden'}`}>
            ...
            <div
              ref={dropdownRef}
              className="min-w-40 max-w-72 max-h-96 overflow-y-auto absolute left-0 -bottom-2 translate-y-full shadow-lg bg-light dark:bg-neutral-800 py-2 rounded-md z-50"
            >
              {options.map(option => (
                <div
                  id={`${name}-${option.id}`}
      ...
      {errorMessage && (
        <p className="text-[10.8px] md:text-xs text-red-600 dark:text-red-400">{errorMessage}</p>
      )}
    </div>
  );
}
