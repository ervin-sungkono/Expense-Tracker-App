...
function TransactionCard({ transaction, readOnly = false }) {
  ...
  const canWriteTransactions = !readOnly && space?.canWriteTransactions;
  ...
  return (
    <div className="relative not-last:border-b border-dark/20 dark:border-white/20 flex items-center">
      <div
        {...handlers()}
        onClick={!readOnly && !isMobile ? () => setShowMenu(true) : null}
        className={`w-full h-full px-2.5 md:px-4 py-2 flex active:bg-neutral-300/30 dark:active:bg-light/10 transition-colors duration-150 ease-in-out ${readOnly ? '' : 'cursor-pointer'}`}
      >
        <div className="w-full flex items-center gap-2.5">
          ...
          <div className="w-full flex items-center gap-1.5">
            <div className="grow flex flex-col gap-1">
              ...
              {!readOnly && (
                <p className="text-xs md:text-sm line-clamp-1 text-dark/80 dark:text-white/80">
                  {remarks}
                </p>
              )}
            </div>
      ...
      {!readOnly && (
        <>
          ...
          <Dialog show={showInfo} hideFn={() => setShowInfo(false)}>
            <InfoTransactionForm transaction={transaction} hideFn={() => setShowInfo(false)} />
          </Dialog>
          <Dialog show={showEdit} hideFn={() => setShowEdit(false)}>
            <AddTransactionForm transaction={transaction} onSubmit={() => setShowEdit(false)} />
          </Dialog>
          <Dialog show={showDelete} hideFn={() => setShowDelete(false)} hideCancelButton>
