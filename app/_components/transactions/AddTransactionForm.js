...
import { dateToLocalInput, getOwnerLabel } from '@lib/utils';
...
export default function AddTransactionForm({ transaction = {}, onSubmit }) {
  ...
  const validateDate = date => {
    return new DateValidator('Date', date).required().date().validate();
  };

  const validateAmount = amount => {
    return new NumberValidator('Amount', amount).required().min(1).validate();
  };

  const validateCategory = categoryId => {
    return new StringValidator('Category', categoryId).required().validate();
  };

  const validateRemarks = remarks => {
    return new StringValidator('Notes', remarks).maxLength(120).validate();
  };
        ...
        <div className="relative w-8 h-8 md:w-10 md:h-10 flex justify-center items-center bg-ocean-blue rounded-full">
          {selectedCategory.icon && (
            <Image
              ...
              src={`./category_icons/${selectedCategory.icon}`}
              alt=""
              fill
            />
          )}
    ...
    <div className="flex flex-col gap-4">
      <div className="flex justify-between items-center">
        ...
        <div
          ...
          className="p-2 rounded-full active:bg-neutral-300/30 dark:active:bg-neutral-700/30 transition-colors ease-in-out"
        >
          <UploadIcon size={20} />
        </div>
      </div>
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col gap-4 mb-6">
          <InputField
            ...
            type={'datetime-local'}
            defaultValue={
              transactionData.date ? dateToLocalInput(transactionData.date) : dateToLocalInput()
            }
          ...
          {selectedCategory?.type === 'DebtLoan' && (
            <InputField
              ...
              placeholder={
                selectedCategory.name === 'Loan' ? "Enter Borrower's name" : 'Enter Lender\s name'
              }
      ...
      <SelectCategoryPage
        ...
        defaultType="Expense"
      />
      <Dialog show={showUpload} hideFn={() => setShowUpload(false)}>
