    ...
    return (
        <div>
            <div onClick={() => setShowDialog(true)} style={style} className={`cursor-pointer w-full h-full flex flex-col ${!isOdd ? 'pr-2' : ''} pb-2`}>
                <div className="relative w-full aspect-video bg-white dark:bg-neutral-800 rounded-t-lg overflow-hidden">
                    {image && <Image src={image} className="object-contain" fill alt={`${name} Image`}/>}
                </div>
                <div className="pt-2.5 pb-4 px-4 rounded-b-lg bg-white dark:bg-neutral-800 grow">
