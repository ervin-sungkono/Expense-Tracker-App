...
const Row = ({ columnIndex, rowIndex, style, data: { items, onShopClick } }) => {
  ...
  if (!item)
    return (
      <div style={style} className="flex justify-center items-center animate-pulse">
        <p>Loading more data..</p>
      </div>
    );

  return <ShopCard shop={item} onClick={onShopClick} isOdd={index % 2 === 1} style={style} />;
};
      ...
      <div className="w-full h-full flex flex-col justify-center items-center text-center gap-2 rounded-lg px-4 py-2.5">
        <div className="relative w-full aspect-[16/10]">
          <Image
            ...
            src={'/not-found.png'}
            alt=""
            className="object-contain opacity-60 saturate-0"
          />
        </div>
          ...
          {({ onItemsRendered, ref }) => (
            <FixedSizeGrid
              ...
              onItemsRendered={props =>
                onItemsRendered({
                  overscanStartIndex:
                    props.overscanRowStartIndex * COLUMN_CNT + props.overscanColumnStartIndex,
                  overscanStopIndex:
                    props.overscanRowStopIndex * COLUMN_CNT + props.overscanColumnStopIndex,
                  visibleStartIndex:
                    props.visibleRowStartIndex * COLUMN_CNT + props.visibleColumnStartIndex,
                  visibleStopIndex:
                    props.visibleRowStopIndex * COLUMN_CNT + props.visibleColumnStopIndex,
                })
              }
            >
              {MemoizedRow}
