  ...
  return (
    <div className="flex flex-col mt-1">
      ...
      <div className="relative w-full h-1.5 md:h-2 bg-neutral-200 dark:bg-neutral-700 rounded-full">
        {remainingBudget < 0 ? (
          <div
            className="absolute top-0 left-0 h-full bg-danger-gradient rounded-full"
            style={{ width: '100%' }}
          ></div>
        ) : (
          <div
            className="absolute top-0 left-0 h-full bg-basic-gradient rounded-full"
            style={{ width: `${(remainingBudget / budgetAmount) * 100}%` }}
          ></div>
        )}
      </div>
      <div className="flex justify-between mt-2 font-medium">
        <p className="text-sm">
          {daysSinceStart < 0
            ? `${daysSinceStart * -1} days to begin`
