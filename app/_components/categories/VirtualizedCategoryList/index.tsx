...
const Row = ({ index, style, data: { items, onCategoryClicked } }) => {
  ...
  return <CategoryCard category={item} onClick={onCategoryClicked} style={style} />;
};
...
const VirtualizedCategoryList = ({ ref, items, onCategoryClicked }) => {
  ...
  if (!items) {
    return (
      <div className="w-full h-full flex flex-col justify-center items-center gap-4 bg-neutral-200 dark:bg-neutral-800 rounded-lg px-4 py-2.5">
        <LoadingSpinner />
        <p className="text-dark/80 dark:text-white/80 text-sm md:text-base text-center">
