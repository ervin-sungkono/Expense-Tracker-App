...
export default function AddCategoryForm({ category = {}, onSubmit }) {
  ...
  useEffect(() => {
    if (category.parentId && parentCategories)
      setSelectedParent(parentCategories.find(pc => pc.id === category.parentId));
  ...
  const validateIcon = icon => {
    return new StringValidator('Icon', icon).required().validate();
  };

  const validateType = type => {
    return new StringValidator('Type', type).required().validate();
  };

  const validateName = name => {
    return new StringValidator('Name', name).required().minLength(3).maxLength(30).validate();
  };
      ...
      <div className="flex items-center gap-2">
        <div className="relative w-8 h-8 md:w-10 md:h-10 flex justify-center items-center bg-ocean-blue rounded-full">
          <Image
            ...
            src={`./category_icons/${selectedIcon}`}
            alt=""
            fill
          />
        </div>
      ...
      <div className="flex items-center gap-2">
        <div className="relative w-8 h-8 md:w-10 md:h-10 flex justify-center items-center bg-ocean-blue rounded-full">
          {selectedParent.icon && (
            <Image
              className="object-contain p-1.5 md:p-2"
              src={`./category_icons/${selectedParent.icon}`}
              alt=""
              fill
            />
          )}
