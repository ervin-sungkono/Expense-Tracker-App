...
export async function generateUserKeyring(passphrase) {
    ...
    const encryptedPrivateKey = await subtle.wrapKey(
        'pkcs8',
        pair.privateKey,
        passphraseKey,
        { name: 'AES-GCM', iv }
    ...
    const fingerprintBytes = await subtle.digest('SHA-256', new TextEncoder().encode(JSON.stringify(publicKeyJwk)));
...
export async function wrapSpaceKey(spaceKey, publicKey) {
    const wrapped = await getCrypto().subtle.wrapKey('raw', spaceKey, publicKey, { name: 'RSA-OAEP' });
...
export async function encryptSpaceKeyForInvitation(spaceKey, secret, metadata) {
    const subtle = getCrypto().subtle;
    const inviteKey = await subtle.importKey('raw', base64ToBytes(secret), { name: 'AES-GCM' }, false, ['wrapKey']);
    const iv = randomBytes(12);
    const wrapped = await subtle.wrapKey(
        'raw',
        spaceKey,
        inviteKey,
        { name: 'AES-GCM', iv, additionalData: new TextEncoder().encode(JSON.stringify(metadata)) }
    );
...
export async function decryptInvitationSpaceKey(envelope, secret, metadata) {
    const subtle = getCrypto().subtle;
    const inviteKey = await subtle.importKey('raw', base64ToBytes(secret), { name: 'AES-GCM' }, false, ['unwrapKey']);
    return subtle.unwrapKey(
        'raw',
        base64ToBytes(envelope.encryptedSpaceKey),
        inviteKey,
        { name: 'AES-GCM', iv: base64ToBytes(envelope.iv), additionalData: new TextEncoder().encode(JSON.stringify(metadata)) },
        { name: 'AES-GCM', length: 256 },
        false,
        ['encrypt', 'decrypt']
    );
