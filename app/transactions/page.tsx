...
export default function Transactions() {
  ...
  useEffect(() => {
    if (defaultCategory && categories) {
      setFilterOptions(prevOptions => ({
        ...prevOptions,
        categoryId: categories.find(c => c.name.toLowerCase() === defaultCategory.toLowerCase())
          ?.id,
      }));
    }
  ...
  useEffect(() => {
    if (transactionQuery && categories && shops) {
      ...
      const transactionData = transactionQuery.map(transaction => ({
        ...transaction,
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex grow mb-2">
          <VirtualizedTransactionList
            ...
            hasNextPage={
              transactionQuery &&
              transactionQuery.length > 0 &&
              transactionQuery.length % PAGE_SIZE === 0
            }
          />
        </div>
      </div>
      <Dialog show={filterMode} hideFn={() => setFilterMode(false)}>
        <FilterTransaction
