...
export async function GET(request) {
    ...
    const { data, error } = await admin.from('space_invitations')
        .select('id,space_id,invited_email,role,encrypted_space_key,key_iv,space_key_version,expires_at,accepted_at,revoked_at,spaces(name)')
    ...
    if (!data || data.invited_email !== email || data.revoked_at || data.accepted_at || new Date(data.expires_at) <= new Date()) {
        return NextResponse.json({ error: 'This invitation is invalid, expired, or belongs to another email.' }, { status: 403 });
    }
...
export async function POST(request) {
    ...
    if (!body.token || !body.wrappedSpaceKey || !body.userKeyVersion) {
        return NextResponse.json({ error: 'Invitation acceptance payload is incomplete.' }, { status: 400 });
    }
    const admin = createSecretClient();
