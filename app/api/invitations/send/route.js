...
export async function POST(request) {
    ...
    if (!process.env.RESEND_API_KEY || !process.env.INVITE_FROM_EMAIL) {
        return NextResponse.json({ error: 'Email is not configured. Copy and send the invitation link instead.' }, { status: 501 });
    }
    const { invitationId, link } = await request.json();
    const { data: invite } = await supabase.from('space_invitations')
        .select('invited_email,spaces(name)').eq('id', invitationId).single();
    ...
    await supabase.from('space_invitations').update({ last_sent_at: new Date().toISOString() }).eq('id', invitationId);
